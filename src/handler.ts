...
export async function handler(ctx: DataHandlerContext<Store, Fields>) {
  ...
  const dbProducts = await ctx.store.find(Product, { where: { id: In(Array.from(products.keys())) } });
  const dbProductMap = new Map(dbProducts.map(product => [product.id, product]));
  products.forEach(product => products.set(product.id, { ...dbProductMap.get(product.id), ...product }));
